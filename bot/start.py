...
async def handle_unknown_text(message: Message, state: FSMContext):
    """
    Обработчик любого неизвестного текста.
    Сначала проверяем, включён ли у пользователя режим ИИ или гибридный режим.
    Если да — делегируем обработку в ai_mode.handle_text_in_ai_mode.
    Если нет — просим использовать кнопки меню.
    """
    user_id = message.from_user.id

    # --- ИСПРАВЛЕНИЕ: проверяем AI/гибридный режим перед показом ошибки ---
    try:
        ...
        async with get_session_maker()() as session:
            ...
            if ai_mode or hybrid_mode:
                # Режим ИИ или гибридный активен — делегируем обработку
                from bot.handlers.ai_mode import handle_text_in_ai_mode
                await handle_text_in_ai_mode(message, state)
                return
    except Exception as e:
        logging.error(f"Ошибка при проверке AI-режима для пользователя {user_id}: {e}")

    # --- Обычное поведение: режим ИИ не активен ---
